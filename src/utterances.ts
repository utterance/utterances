...
async function bootstrap() {
	...
	const submit = async (markdown: string) => {
		...
		timeline.insertComment(comment, true, sort);
		newCommentComponent.clear();
	};

	const newCommentComponent = new NewCommentComponent(user, submit);
	let firstbox = timeline.element.firstChild.nextSibling.nextSibling;
	if (page.sort === 'new') {
		timeline.element.insertBefore(newCommentComponent.element, firstbox);
	} else {
		timeline.element.append(newCommentComponent.element)
	}
...
async function renderComments(issue: Issue, timeline: TimelineComponent) {
	...
	const renderPage = (page: IssueComment[]) => {
		for (const comment of page) {
			timeline.insertComment(comment, false, sort);
		}
	};
