...
export class TimelineComponent {
	...
	public insertComment(comment: IssueComment, incrementCount: boolean, sortNew: boolean) {
		const component = new CommentComponent(
			comment,
			this.user ? this.user.login : null
		);
		
		const oldToNew = this.timeline.findIndex(x => x.comment.id >= comment.id);
		const newToOld = this.timeline.findIndex(x => x.comment.id <= comment.id);
		const index = sortNew ? newToOld : oldToNew;
